...
```bash
PHASE_INFO=$(node "c:/Dev/poc/plantz-gsd/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "c:/Dev/poc/plantz-gsd/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "c:/Dev/poc/plantz-gsd/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
