...
```bash
# Get state snapshot
node "C:/Dev/poc/plantz-gsd/.claude/get-shit-done/bin/gsd-tools.cjs" state json 2>/dev/null || echo "{}"
```
...
```bash
node "C:/Dev/poc/plantz-gsd/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: defer incomplete Phase {src} items to backlog"
```
