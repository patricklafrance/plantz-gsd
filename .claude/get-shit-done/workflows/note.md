...
- **Global scope**: `C:/Dev/poc/plantz-gsd/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Dev/poc/plantz-gsd/.claude/notes/`)
...
2. Glob `C:/Dev/poc/plantz-gsd/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Dev/poc/plantz-gsd/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Dev/poc/plantz-gsd/.claude/notes/` — works in any directory
